...
/*   Updated: 2023/10/09 15:42:54 by aaslan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_handle_line(t_data *data, char *line, int readed_byte, int *i)
{
	if (line[*i] == '\n' || readed_byte == 0)
	{
		data->config_total_line_count++;
		line[*i + 1] = '\0';
		if (!ft_is_empty_line(line))
		{
			data->config_full_line_count++;
			if (data->config_full_line_count == 7)
				data->map_starting_line = data->config_total_line_count;
		}
		else
			data->config_empty_line_count++;
		ft_clear_line(line);
		*i = -1;
	}
}
...
void ft_set_line_count(t_data *data)
{
	...
	if (data->config_full_line_count < 9)
