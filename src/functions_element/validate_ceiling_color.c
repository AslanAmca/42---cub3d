...
static void validate_comma(t_data *data)
{
	...
	if (count != 2)
		print_error(data, "C take a value in RGB format. \
...
static void validate_red_color(t_data *data)
{
	...
	if (red_len == 0)
		print_error(data, "Red color of C can't be empty.");
	if (red_len > 3)
		print_error(data, "Red color of C must be between 1-3 characters. (1 - 10 - 100).");
	i = 0;
	while (i < red_len)
	{
		if (!ft_is_digit(red_string[i]))
			print_error(data, "Red color of C must consist of digit only. (0 - 9)");
		i++;
	}
	red = ft_atoi(red_string);
	if (red < 0 || red > 255)
		print_error(data, "Red color of C must be a value between 0-255.");
...
static void validate_green_color(t_data *data)
{
	...
	if (green_len == 0)
		print_error(data, "Green color of C can't be empty.");
	if (green_len > 3)
		print_error(data, "Green color of C must be between 1-3 characters. (1 - 10 - 100).");
	i = 0;
	while (i < green_len)
	{
		if (!ft_is_digit(green_string[i]))
			print_error(data, "Green color of C must consist of digit only. (0 - 9)");
		i++;
	}
	green = ft_atoi(green_string);
	if (green < 0 || green > 255)
		print_error(data, "Green color of C must be a value between 0-255.");
...
static void validate_blue_color(t_data *data)
{
	...
	if (blue_len == 0)
		print_error(data, "Blue color of C can't be empty.");
	if (blue_len > 3)
		print_error(data, "Blue color of C must be between 1-3 characters. (1 - 10 - 100).");
	i = 0;
	while (i < blue_len)
	{
		if (!ft_is_digit(blue_string[i]))
			print_error(data, "Blue color of C must consist of digit only. (0 - 9)");
		i++;
	}
	blue = ft_atoi(blue_string);
	if (blue < 0 || blue > 255)
		print_error(data, "Blue color of C must be a value between 0-255.");
...
void validate_ceiling_color(t_data *data, char *line)
{
	...
	if (line[0] == 'C')
	{
		data->ceiling_color->string = ft_ft_strtrim_start(line + 1, " \t");
		if (*data->ceiling_color->string == '\0')
			print_error(data, "C element cant' be empty.");
		validate_comma(data);
